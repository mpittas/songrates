    ...
    <main className="min-h-screen bg-[#050507]">
      <div className="max-w-4xl mx-auto px-6 pt-20 pb-20">
        ...
        <div className="flex flex-col md:flex-row items-center md:items-start gap-8 mb-12">
          <div className="relative w-48 h-48 md:w-56 md:h-56 shrink-0 bg-neutral-800 shadow-2xl rounded-sm overflow-hidden border border-white/5">
            <OptimizedImage
              src={imageUrl}
              alt={playlist.name}
              fill
          ...
          <div className="divide-y divide-white/5">
            {playlist.tracks.map((song, i) => {
              ...
              const albumSlug = song.albumName
                ? createSlug(song.albumName, (song as any).albumId || "search")
                : null;
              ...
              return (
                <div
                  key={song.id + i}
                  className="flex items-center gap-4 py-3 px-2 group hover:bg-neutral-900/40 transition-colors rounded-sm"
                >
                  {/* Number */}
                  ...
                  {/* Artwork */}
                  {song.artworkUrl ? (
                    <div className="relative w-12 h-12 shrink-0 bg-neutral-800 rounded-sm overflow-hidden">
                      <OptimizedImage
                    ...
                    <div className="w-12 h-12 shrink-0 bg-neutral-800 rounded-sm flex items-center justify-center">
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-center">
                    {trackLink ? (
                      <Link
                        href={trackLink}
                        className="text-sm font-medium text-neutral-200 truncate group-hover:text-[#00f0ff] transition-colors inline-block"
                      >
                        {song.name}
                      </Link>
                    ) : (
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-neutral-500 truncate mt-0.5">
                      ...
                      {song.albumName && (
                        <>
                          ...
                          <span className="truncate text-neutral-600 group-hover:text-neutral-400 transition-colors">
                            {song.albumName}
                          </span>
                        </>
                      )}
